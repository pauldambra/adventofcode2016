...
# For the data to not be suspiscious, it needs to have certain
# properties; purely random data will be detected as tampering.
# To generate appropriate random data, you'll need to use a modified
# dragon curve.
...
# exactly half as long as the original. If the length of the checksum
# is even, repeat the process until you end up with a checksum with
# an odd length.

# For example, suppose we want to fill a disk of length 12, and when
# we finally generate a string of at least length 12, the first 12
