...
# open; after going DUR, all doors lock. (Fortunately, your actual passcode
# is not hijkl).
...
describe "day 17 - part one" do
  it "can hash passcodes" do
    expect(VaultCracker.directions("hijkl")).to eq up: :open, down: :open, left: :open, right: :closed
  end

  it "does not walk off the map" do
    vault = Vault.new("hijkl", '', [0,0])

    expect(vault.next_options).to eq [Vault.new("hijkl", "D", [0,1])]
  end

# Next, having gone only one step (down, or D), you find the hash of hijklD.
# This produces f2bc, which indicates that you can go back up, left (but that's
# a wall), or right.
  it "takes the second step" do
    vault = Vault.new("hijkl", "D", [0,1])
    expect(vault.next_options).to eq [
      Vault.new("hijkl", "DR", [1,1]),
      Vault.new("hijkl", "DU", [0,0])
    ]
  end

  it "can take the third and fourth steps" do
    expect(Vault.new("hijkl", "DR", [1,1]).next_options).to eq []
    expect(Vault.new("hijkl", "DU", [0,0]).next_options).to eq [
      Vault.new("hijkl", "DUR", [1,0])
    ]
    expect(Vault.new("hijkl", "DUR", [1,0]).next_options).to eq []
  end

  it "can build a tree of options for a dead end condition" do
    simulator = VaultSimulator.new([3,3])
    vault = simulator.shortest_path_from Vault.new("hijkl", '', [0,0])
    expect(vault).to eq nil
  end

  it "can find the shortest path for ihgpwlah" do
    simulator = VaultSimulator.new([3,3])
    
    vault = simulator.shortest_path_from Vault.new("ihgpwlah", '', [0,0])
    expect(vault.path).to eq "DDRRRD"
  end

  it "can find the shortest path for kglvqrro" do
    simulator = VaultSimulator.new([3,3])
    
    vault = simulator.shortest_path_from Vault.new("kglvqrro", '', [0,0])
    expect(vault.path).to eq "DDUDRLRRUDRD"
  end

  it "can find the shortest path for ulqzkmiv" do
    simulator = VaultSimulator.new([3,3])
    
    vault = simulator.shortest_path_from Vault.new("ulqzkmiv", '', [0,0])
    expect(vault.path).to eq "DRURDRUDDLLDLUURRDULRLDUUDDDRR"
  end

  it "can find the shortest path for puzzle input" do
    simulator = VaultSimulator.new([3,3])
    
    vault = simulator.shortest_path_from Vault.new("mmsxrhfx", '', [0,0])
    p "found shortest path as #{vault.path}"
  end
end
