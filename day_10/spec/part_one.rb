...
# when it has two microchips, and once it does, it gives
# each one to a different bot or puts it in a marked "output" bin.
...
# that is responsible for comparing value-61 microchips 
# with value-17 microchips?

describe "day 10 part one" do
  it "can defuckulate the example" do
    factory = BalanceBotFactory.new(%{
value 5 goes to bot 2
bot 2 gives low to bot 1 and high to bot 0
value 3 goes to bot 1
bot 1 gives low to output 1 and high to bot 0
...
value 2 goes to bot 2
    ...
    expect(factory.bot_that_compares('value-5', 'value-2')).to eq 2
  end

  it "can defuckulate the puzzle input" do
    factory = BalanceBotFactory.new(File.read(__dir__+'/puzzle_input.txt'))
    target = factory.bot_that_compares('value-61', 'value-17')
    puts "bot that compares 17 and 61 is #{target}"
  end
end