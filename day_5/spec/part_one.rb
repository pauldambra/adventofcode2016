...
# by finding the MD5 hash of some Door ID (your puzzle input) and an increasing integer index 
# (starting with 0).
...
module DoorHacker
  ...
  def hack(door_id)
    ...
    while password_chars.length < 8 do
      ...
      password_chars.push(hash[5]) if hash.start_with?('00000') 
      index += 1
    end
...
describe "day 5 - part one - how about a nice game of chess" do
  it "can find the example password" do
    expect(DoorHacker.hack 'abc').to eq('18f47a30')
  end

  it "solve the problem" do
    p "puzzle solution is #{DoorHacker.hack 'ojvtpuvg'}"
  end
end
