...
# is an implementation of two-factor authentication after a
...
# Then, it displays a code on a little screen, and you type that
...
# out how it works. Now you just have to work out what
# the screen would have displayed.

# The magnetic strip on the card you swiped encodes a series
# of instructions for the screen; these instructions are your
# puzzle input. The screen is 50 pixels wide and 6 pixels tall,
# all of which start off, and is capable of three somewhat
# peculiar operations:
...
# down by one pixel, causing the bottom pixel to wrap
# back to the top:
...
describe "day 8 part 1" do
  ...
  it "can solve the simple example" do
    display = Display.new(3, 7)
    display.operate('rect 3x2')
    expect(display.pixels_lit).to eq 6

    display.operate('rotate column x=1 by 1')
    expect(display.row(0)).to eq ['#','.','#','.','.','.','.']

    display.operate('rotate row y=0 by 4')
    expect(display.show).to eq "....#.#\n###....\n.#.....\n\n"
  ...
  it "can solve the puzzle input" do
    display = Display.new(6, 50)
    File.readlines(__dir__ + '/puzzle_input.txt')
        .each {|o| display.operate(o) }

    display.show
    p "display has #{display.pixels_lit} pixels lit"
  end
end
