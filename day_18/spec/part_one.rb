...
# tile 5 is based on tiles D, E, and an imaginary "safe" tile.
...
describe "day 18 - part one" do
  it "can solve for ..^^." do
    r = Floor.new("..^^.").generate_next_n_rows(2)
    expect(r).to eq [
      '..^^.',
      '.^^^^',
      '^^..^'
    ]
    expect(Floor.safe_tile_count(r)).to eq 6
  end

  it "can generate ten rows for .^^.^.^^^^" do
    r = Floor.new(".^^.^.^^^^").generate_next_n_rows(9)
    expect(r).to eq [
".^^.^.^^^^",
"^^^...^..^",
"^.^^.^.^^.",
...
"^^.^^^..^^"
    ...
    expect(Floor.safe_tile_count(r)).to eq 38
  end
  ...
  it "can solve part 2" do
    f = Floor.new('^..^^.^^^..^^.^...^^^^^....^.^..^^^.^.^.^^...^.^.^.^.^^.....^.^^.^.^.^.^.^.^^..^^^^^...^.....^....^.')
    r = f.generate_next_n_rows(399999)
    p "there are #{Floor.safe_tile_count(r)} safe tiles"
  end
end
